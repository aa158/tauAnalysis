...
float getVisMass(const std::vector<const reco::GenParticle*>& daughters, int status)
{
  float mass = 0;

  for ( std::vector<const reco::GenParticle*>::const_iterator daughter = daughters.begin();
        daughter != daughters.end(); ++daughter ) {
    // std::cout << "is Neutrino? " << isNeutrino(*daughter) << std::endl;
    if ( (status == -1 || (*daughter)->status() == status) && !isNeutrino(*daughter) ) {
      //std::cout << "adding daughter: pdgId = " << (*daughter)->pdgId() << ", Pt = " << (*daughter)->pt() << ","
      //          << " eta = " << (*daughter)->eta() << ", phi = " << (*daughter)->phi()*180./TMath::Pi() << std::endl;
      mass += (*daughter)->mass();
...
float getVisMass(const reco::GenParticle* genLeg)
{
  std::vector<const reco::GenParticle*> stableDaughters;
  findDaughters(genLeg, stableDaughters, 1);

  float mass = getVisMass(stableDaughters);

  return mass;
}

float getNuMass(const std::vector<const reco::GenParticle*>& daughters, int status)
{
  float mass = 0;
...
float getNuMass(const reco::GenParticle* genLeg)
{
  std::vector<const reco::GenParticle*> stableDaughters;
  findDaughters(genLeg, stableDaughters, 1);

  float mass = getNuMass(stableDaughters);
