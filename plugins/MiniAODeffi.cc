	...
	jetToken_(consumes<pat::JetCollection>(iConfig.getParameter<edm::InputTag>("jets")))
{
	...
	tree->Branch("genNuMass",&genMass_,"genMass_/I");
	tree->Branch("genMassDirect",&genMassDirect_,"genMassDirect_/I");
...
MiniAODeffi::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	for (size_t i = 0; i < GenObjects.size(); ++i) {
		...
		genNuMass_=getNuMass(GenObjects[i]);
		genMassDirect_=GenObjects[i]->mass();
